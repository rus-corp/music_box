...
def relation_exist(user_id: int, client_group_id: int, status: bool = True):
  if not status:
    return JSONResponse(content=f'User with id {user_id} has not in group {client_group_id}', status_code=400)
  else:
    return JSONResponse(content=f'User with id {user_id} has in group {client_group_id}', status_code=400)