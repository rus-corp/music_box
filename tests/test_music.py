...
async def test_create_collection(ac: AsyncClient, create_test_super_user_access_token, create_test_user_token, create_test_redactor_token):
  for item in main_group_data[:3]:
    super_user_req = await ac.post('/music/main_group/group_collections', headers={"Authorization": f"Bearer {create_test_super_user_access_token['access_token']}"}, json=item)
  ...
  for item2 in main_group_data[3: -1]:
    redactor_req = await ac.post('/music/main_group/group_collections', headers={"Authorization": f"Bearer {create_test_redactor_token['access_token']}"}, json=item2)
  ...
  redactor_bad_req = await ac.post('/music/main_group/group_collections', headers={"Authorization": f"Bearer {create_test_redactor_token['access_token']}"}, json=main_group_data[-1])
  assert redactor_bad_req.status_code == 400
  client_req = await ac.post('/music/main_group/group_collections', headers={"Authorization": f"Bearer {create_test_user_token['access_token']}"}, json=main_group_data[-2])
  assert client_req.status_code == 405
  
  collection_db_data = await ac.get('/music/main_group/group_without_track_collections', headers={"Authorization": f"Bearer {create_test_redactor_token['access_token']}"})
...
async def test_get_all_colelctions_without_track_group(ac: AsyncClient, create_test_super_user_access_token, create_test_user_token, create_test_redactor_token):
  redactor_req = await ac.get('/music/main_group/group_without_track_collections', headers={"Authorization": f"Bearer {create_test_super_user_access_token['access_token']}"})
  assert redactor_req.status_code == 200
  redactor_data = redactor_req.json()
  assert len(redactor_data) == len(main_group_data) - 1
  super_user_req = await ac.get('/music/main_group/group_without_track_collections', headers={"Authorization": f"Bearer {create_test_redactor_token['access_token']}"})
  assert super_user_req.status_code == 200
  super_user_data = super_user_req.json()
  assert len(main_group_data) - 1 == len(super_user_data)
  user_req = await ac.get('/music/main_group/group_without_track_collections', headers={"Authorization": f"Bearer {create_test_user_token['access_token']}"})
  assert user_req.status_code == 405
...
async def test_get_one_colelction_without_track_group(ac: AsyncClient, create_test_super_user_access_token, create_test_user_token, create_test_redactor_token):
  redactor_req = await ac.get('/music/main_group/group_collection/2', headers={"Authorization": f"Bearer {create_test_redactor_token['access_token']}"})
  assert redactor_req.status_code == 200
  redactor_data = redactor_req.json()
  assert redactor_data['id'] == 2
  assert redactor_data['group_name'] == main_group_data[1]['group_name']
  super_user_req = await ac.get('/music/main_group/group_collection/1', headers={"Authorization": f"Bearer {create_test_super_user_access_token['access_token']}"})
  assert super_user_req.status_code == 200
  super_user_data = super_user_req.json()
  assert super_user_data['id'] == 1
  assert super_user_data['group_name'] == main_group_data[0]['group_name']
  user_req = await ac.get('/music/main_group/group_collection/1', headers={"Authorization": f"Bearer {create_test_user_token['access_token']}"})
  assert user_req.status_code == 405
...
async def test_update_collection(ac: AsyncClient, create_test_super_user_access_token, create_test_user_token, create_test_redactor_token):
  ...
  super_req = await ac.patch('/music/main_group/group_collections', headers={"Authorization": f"Bearer {create_test_super_user_access_token['access_token']}"}, json=data1)
  ...
  redactor_update_req = await ac.patch('/music/main_group/group_collections', headers={"Authorization": f"Bearer {create_test_redactor_token['access_token']}"}, json=data2)
  ...
  redactor_update_req = await ac.get('/music/main_group/group_collection/2', headers={"Authorization": f"Bearer {create_test_redactor_token['access_token']}"})
  assert redactor_update_req.status_code == 200
  redactor_update_req_db = redactor_update_req.json()
  assert redactor_update_req_db['group_name'] == data2['name']
  
  data3 = {'id': 1, 'name': 'NEW THERD NAME'}
  user_req = await ac.patch('/music/main_group/group_collections', headers={"Authorization": f"Bearer {create_test_user_token['access_token']}"}, json=data3)
  assert user_req.status_code == 405
...
async def test_create_track_collcetions(ac: AsyncClient, create_test_super_user_access_token, create_test_user_token, create_test_redactor_token):
  ...
  for item2 in track_group_data[5 : -2]:
    redactor_req = await ac.post('/music/track_group/',
  ...
  user_bad_req = await ac.post('/music/track_group/',
                               headers={"Authorization": f"Bearer {create_test_user_token['access_token']}"},
                               json=track_group_data[4])
  assert user_bad_req.status_code == 405
  
  
  redactor_bad_req1 = await ac.post('/music/track_group/',
                                headers={"Authorization": f"Bearer {create_test_redactor_token['access_token']}"},
                                json=track_group_data[-1])
  assert redactor_bad_req1.status_code == 422
  
  redactor_bad_req2 = await ac.post('/music/track_group/',
                                headers={"Authorization": f"Bearer {create_test_redactor_token['access_token']}"},
